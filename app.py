...
<style>
    ...
    /* Custom Button Styles for Page Links */
...
if os.path.exists(FEEDBACK_FILE) and os.path.getsize(FEEDBACK_FILE) > 2:
    ...
    if feedback_data:
        ...
        with col1:
            st.markdown(f'<div class="stat-card"><h3>{len(df)}</h3><p>Total Feedback</p></div>', unsafe_allow_html=True)
        with col2:
            st.markdown(f'<div class="stat-card"><h3>{df["rating"].mean():.1f}⭐</h3><p>Average Rating</p></div>', unsafe_allow_html=True)
        with col3:
