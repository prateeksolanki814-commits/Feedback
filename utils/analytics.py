...
def show_analytics():
    ...
    with c2: st.metric("Average Rating", f"{df['rating'].mean():.1f} ⭐")
    with c3:
    ...
    with col1:
        ...
        fig = px.bar(rating_counts, x='Rating', y='Count', color='Rating', text='Count',
    ...
    with col2:
        ...
        fig = px.pie(category_counts, values='Count', names='Category', hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
